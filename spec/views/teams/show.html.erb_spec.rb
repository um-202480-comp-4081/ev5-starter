...
RSpec.describe 'teams/show.html.erb' do
  before do
    assign :team, build_stubbed(:team)

    allow(view).to receive(:link_to).and_call_original
    allow(view).to receive(:teams_path).and_call_original
  ...
  it 'uses appropriate route helper(s)' do
    expect(view).to have_received(:teams_path).once
    expect(view).to have_received(:edit_team_path).once
  end

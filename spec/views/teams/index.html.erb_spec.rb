...
RSpec.describe 'teams/index.html.erb' do
  before do
    assign :teams, build_stubbed_list(:team, 2)
    ...
    allow(view).to receive(:new_team_path).and_call_original
    allow(view).to receive(:edit_team_path).and_call_original
  ...
  it 'uses appropriate route helper(s)' do
    expect(view).to have_received(:team_path).exactly(4).times
    expect(view).to have_received(:new_team_path).exactly(1).times
    expect(view).to have_received(:edit_team_path).exactly(2).times
  end
